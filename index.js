...
// ── BigQuery (catálogo de merchants) ────────────────────────
// Variables de entorno requeridas: BQ_PROJECT_ID, BQ_DATASET, (opcional) BQ_TABLE (default 'dim_merchants')
// Si no usas credenciales por defecto de Cloud Run, agrega GCP_SERVICE_ACCOUNT_JSON con el JSON de la service account.
...
const BQ_TABLE_FQN = `\`${process.env.BQ_PROJECT_ID}.${process.env.BQ_DATASET}.${process.env.BQ_TABLE || 'dim_merchants'}\``;
...
// Constantes reutilizables para evitar el bug de initial_option / findOpt (ya lo cazamos una vez con Tipo)
const CHANNEL_OPTIONS = ['Online', 'Offline', 'Ambos'].map(t => opt(t, t));
const BUSINESS_LINE_OPTIONS = ['BNPL', 'VC', 'PC', 'Walmart', 'Ali', 'Todos', 'Combo'].map(t => opt(t, t));
...
// ── Helpers de catálogo de merchants (BigQuery) ─────────────
...
async function lookupMerchants(ids) {
  ...
  const query = `
    SELECT merchant_id, channel, business_line
    FROM ${BQ_TABLE_FQN}
    WHERE merchant_id IN UNNEST(@ids)
  `;
  const [rows] = await bigquery.query({ query, params: { ids } });
  const found = {};
  for (const row of rows) {
    found[String(row.merchant_id)] = { channel: row.channel, businessLine: row.business_line };
  }
  const missing = ids.filter(id => !found[id]);
  return { found, missing };
}

// Regla de "Ambos" / "Combo": si todos los merchants comparten canal/business line, se usa ese valor;
// si hay mezcla, cae a 'Ambos' (canal) o 'Combo' (business line, opción ya existente en el dropdown).
function computeChannelAndBusinessLine(ids, catalogMap) {
  const channels = ids.map(id => catalogMap[id].channel);
  const businessLines = ids.map(id => catalogMap[id].businessLine);
  const channel = channels.every(c => c === channels[0]) ? channels[0] : 'Ambos';
  const businessLine = businessLines.every(b => b === businessLines[0]) ? businessLines[0] : 'Combo';
  return { channel, businessLine };
}
...
function blocksPage2(a) {
  return [
    ...
    { type: 'input', block_id: 'b_merchant_id_compra', label: { type: 'plain_text', text: 'Merchant id(s) [compra]' },
      element: { type: 'plain_text_input', action_id: 'merchant_id_compra', multiline: true,
        placeholder: { type: 'plain_text', text: 'Uno por línea o separados por coma' },
        ...(a.merchant_id_compra ? { initial_value: a.merchant_id_compra } : {}) } },
    // Canal: se autollena a partir del catálogo de merchants al darle "Siguiente" en esta página,
    // pero sigue siendo editable — si el usuario lo cambia a mano, se respeta hasta que vuelva a
    // editar la lista de Merchant ID(s), momento en el que se recalcula desde cero.
    { type: 'input', block_id: 'b_merchant_channel', label: { type: 'plain_text', text: 'Merchant channel' },
      element: { type: 'static_select', action_id: 'merchant_channel',
        ...
        ...(a.merchant_channel ? { initial_option: findOpt(CHANNEL_OPTIONS, a.merchant_channel) } : {}) } },
  ];
}
...
function blocksPage5(a) {
  const blocks = [
    // Business Line: igual que Canal, se autollena desde el catálogo (calculado en la Página 2)
    // pero se puede editar aquí; si el usuario la cambia a mano se respeta esa edición.
    { type: 'input', block_id: 'b_business_line', label: { type: 'plain_text', text: 'Business line' },
      element: { type: 'static_select', action_id: 'business_line',
        ...
        ...(a.business_line ? { initial_option: findOpt(BUSINESS_LINE_OPTIONS, a.business_line) } : {}) } },
...
app.view('promo_step_view', async ({ ack, body }) => {
  ...
  // ── Validación de Merchant ID(s) + autollenado de Canal / Business Line ──
  // Se dispara al salir de la Página 2 (donde vive merchant_id_compra).
  if (meta.step === 'p2') {
    const ids = parseMerchantIds(merged.merchant_id_compra);

    if (ids.length > 0) {
      let lookup;
      try {
        lookup = await lookupMerchants(ids);
      } catch (err) {
        console.error('Error consultando el catálogo de merchants en BigQuery:', err);
        await ack({
          response_action: 'errors',
          errors: { b_merchant_id_compra: 'No se pudo validar el/los Merchant ID en este momento. Intenta de nuevo en unos segundos.' },
        });
        return;
      }
      ...
      if (lookup.missing.length > 0) {
        await ack({
          response_action: 'errors',
          errors: { b_merchant_id_compra: `Merchant ID no encontrado: ${lookup.missing.join(', ')}` },
        });
        return;
      }

      const idsKey = ids.slice().sort().join(',');
      const idsChanged = idsKey !== merged.merchant_ids_key_prev;
      const { channel: computedChannel, businessLine: computedBusinessLine } = computeChannelAndBusinessLine(ids, lookup.found);

      if (idsChanged) {
        // Los merchants cambiaron respecto a la última vez: se resetea cualquier edición manual
        // previa y se recalcula todo desde cero (regla acordada: re-editar merchants reinicia el autofill).
        merged.canal_edited_manually = false;
        merged.business_line_edited_manually = false;
        merged.merchant_channel = computedChannel;
        merged.business_line = computedBusinessLine;
      } else {
        // Mismos merchants que la vez anterior — si el usuario cambió Canal a mano
        // (difiere del último valor auto-calculado), se respeta su edición.
        if (merged.merchant_channel !== merged.merchant_channel_auto) {
          merged.canal_edited_manually = true;
        } else {
          merged.merchant_channel = computedChannel;
        }
        // Business Line se resuelve de forma análoga al salir de la Página 5 (ver más abajo).
      }

      merged.merchant_channel_auto = computedChannel;
      merged.business_line_auto = computedBusinessLine;
      merged.merchant_ids_key_prev = idsKey;
    }
  }

  // ── Detectar edición manual de Business Line al salir de la Página 5 ──
  if (meta.step === 'p5') {
    if (merged.business_line_auto && merged.business_line !== merged.business_line_auto) {
      merged.business_line_edited_manually = true;
    }
  }
